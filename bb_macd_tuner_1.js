...
function backtestBBMacd(
    ...
    let trades = [];
    let position = null;
    let lowerTouch = null; // penanda kalau harga sudah sentuh lower BB
    ...
    for (let i = 0; i < closes.length; i++) {
        ...
        // --- Tandai saat harga menyentuh lower BB ---
        if (!position && price <= bb.lower) {
            lowerTouch = i;
        }

        // --- Entry: setelah harga sentuh lower BB, lalu hist > 0 ---
        if (!position && lowerTouch !== null && hist > 0) {
            position = { entryPrice: price, entryIndex: i };
            lowerTouch = null; // reset karena sudah entry
        }
